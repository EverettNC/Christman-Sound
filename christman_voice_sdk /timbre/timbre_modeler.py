...
class VoiceProfile:
    ...
    captured_signature: Optional[Dict] = None
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "captured_signature": self.captured_signature,
        }
...
class TimbreModeler:
    ...
    - Spectral envelope
    - Voice capture client profile (frequency signature)
    ...
    def __init__(
        ...
        # Load latest voice capture profile if one exists
        try:
            from integration.voice_capture_client import load_profile, list_profiles
            profiles = list_profiles()
            if profiles:
                self.captured_signature = load_profile(profiles[-1])
                logger.info(f"Loaded captured voice signature: {profiles[-1]}")
            else:
                logger.info("No captured voice profiles found — will use x-vector only.")
        except Exception as e:
            logger.warning(f"Voice capture client unavailable: {e}")
    ...
    def _extract_x_vector(self, segments: List[AudioSegment]) -> np.ndarray:
        ...
        Blends captured voice frequency signature when available.
        """
        try:
            ...
            # Blend captured frequency signature into x-vector if available
            if self.captured_signature is not None:
                pitch_hz = self.captured_signature.get("fundamental_hz", 150.0)
                pitch_norm = np.clip((pitch_hz - 80.0) / (300.0 - 80.0), 0.0, 1.0)
                x_vec = x_vec * 0.85 + pitch_norm * 0.15
                logger.info(f"Blended captured voice signature — pitch: {pitch_hz:.1f} Hz")
    ...
    def _extract_f0_profile(self, segments: List[AudioSegment]) -> Dict:
        """Extract fundamental frequency profile using native DSP."""
        all_f0 = []
    ...
    def _extract_formants(self, segments: List[AudioSegment]) -> Tuple[float, float, float]:
        """Extract formant frequencies using native LPC."""
        all_formants = []
        ...
        return (
            float(np.mean(all_formants[:, 0])),
            float(np.mean(all_formants[:, 1])),
            float(np.mean(all_formants[:, 2]))
        )

    def _extract_voice_quality(self, segments: List[AudioSegment]) -> Dict:
        """Extract voice quality metrics using ToneScore engine."""
...
if __name__ == "__main__":
    ...
    if profile.captured_signature:
        print(f"Captured pitch: {profile.captured_signature.get('fundamental_hz', 'N/A')} Hz")
