...
# ── Logger (Rule 6: fail loud with context) ──
...
# Two homes: inside the package (wheel installs) or at the repo root (editable
# install / repo checkout, where the .so is built). First one that exists wins;
# neither existing fails loud below, exactly as before.
_dsp_candidates = (
    Path(__file__).parent / "christman_dsp.so",
